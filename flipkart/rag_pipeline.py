...
class RAGChainBuilder:
    def __init__(self, vector_store) -> None:
        ...
        self.model = ChatGroq(model=Config.RAG_MODEL,
                              temperature=0.5)
        self.history_store = {}
    ...
    def build_chain(self) -> RunnableWithMessageHistory:
        ...
        qa_prompt = ChatPromptTemplate.from_messages([
            ("system", """You're an e-commerce bot answering product-related queries using reviews and titles.
                          Stick to context. Be concise and helpful. Skip the initial summmary and output the list of suggestions directly.\n\nCONTEXT:\n{context}\n\nQUESTION: {input}"""),
            MessagesPlaceholder(variable_name="chat_history"),
            ("human", "{input}")
        ])
